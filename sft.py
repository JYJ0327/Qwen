...
def train_epoch(epoch):
    ...
    for step, (X, Y,loss_mask) in enumerate(train_loader):
        ...
        with ctx:
            logits = model(X, Y)
            loss = F.cross_entropy(logits['last_hidden_state'].view(-1, logits.size(-1)), Y.view(-1), ignore_index=0,reduce=False)
...
def init_model():
    ...
    if init_from == "scratch":
        ...
        # model = Qwen2Model.from_pretrained('./out/pretrain/Qwen1.5-0.5B')
...
if __name__=="__main__":
   ...
   # various inits, derived attributes, I/O setup
    ...
    print(df)
    tokenizer=Qwen2Tokenizer.from_pretrained('./out/pretrain/Qwen1.5-0.5B')
